...
import { getCustomerLevel } from '#/api/crm/statistics/portrait';
import { $t } from '#/locales';
import { DICT_TYPE, getDictLabel } from '#/utils';

import { levelColumns } from '../data';
...
const chartData = ref<CrmStatisticsPortraitApi.CustomerLevel[]>([]);
...
function renderLeftPie() {
  const data = chartData.value.map((item, index) => ({
    name: getDictLabel(DICT_TYPE.CRM_CUSTOMER_LEVEL, item.level),
...
async function loadData() {
  ...
  try {
    ...
    const tableRows = res.map((item) => ({
      ...item,
      levelName: getDictLabel(DICT_TYPE.CRM_CUSTOMER_LEVEL, item.level),
      levelPortion: calcPortion(item.customerCount, totalCustomer),
