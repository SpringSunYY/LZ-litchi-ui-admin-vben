...
import { followCustomerCountRankColumns } from '../data';
...
const [Grid, gridApi] = useVbenVxeGrid({
  gridOptions: {
    columns: followCustomerCountRankColumns,
    height: 400,
    data: [],
  },
});
...
async function renderChart() {
  ...
  renderEcharts({
    ...
    series: [
      {
        name: $t('crm.statistics.rank.followCustomerCount'),
        type: 'bar',
...
async function loadData() {
  ...
  loading.value = true;
  try {
    const res = await getFollowCustomerCountRank({
