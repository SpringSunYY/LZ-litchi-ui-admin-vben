...
import { getCustomerCountRank } from '#/api/crm/statistics/rank';
import { $t } from '#/locales';

import { customerCountRankColumns } from '../data';
...
const [Grid, gridApi] = useVbenVxeGrid({
  gridOptions: {
    columns: customerCountRankColumns,
    height: 400,
    data: [],
  },
});
...
async function renderChart() {
  ...
  renderEcharts({
    ...
    series: [
      {
        name: $t('crm.statistics.rank.customerCount'),
        type: 'bar',
        itemStyle: { color: '#ff8183' },
...
async function loadData() {
  ...
  loading.value = true;
  try {
    const res = await getCustomerCountRank({
