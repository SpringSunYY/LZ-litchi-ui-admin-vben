...
import { getContractPriceRank } from '#/api/crm/statistics/rank';
import { $t } from '#/locales';

import { contractPriceRankColumns } from '../data';
...
const [Grid, gridApi] = useVbenVxeGrid({
  gridOptions: {
    columns: contractPriceRankColumns,
    height: 400,
    data: [],
  },
});
...
async function renderChart() {
  ...
  renderEcharts({
    ...
    series: [
      {
        name: $t('crm.statistics.rank.contractPrice'),
        type: 'bar',
...
async function loadData() {
  ...
  loading.value = true;
  try {
    const res = await getContractPriceRank({
