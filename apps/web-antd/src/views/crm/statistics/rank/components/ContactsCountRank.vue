...
import { getContactsCountRank } from '#/api/crm/statistics/rank';
import { $t } from '#/locales';

import { contactsCountRankColumns } from '../data';
...
const [Grid, gridApi] = useVbenVxeGrid({
  gridOptions: {
    columns: contactsCountRankColumns,
    height: 400,
    data: [],
  },
});
...
async function renderChart() {
  ...
  renderEcharts({
    ...
    series: [
      {
        name: $t('crm.statistics.rank.contactsCount'),
        type: 'bar',
        itemStyle: { color: '#c87aeb' },
...
async function loadData() {
  ...
  loading.value = true;
  try {
    const res = await getContactsCountRank({
