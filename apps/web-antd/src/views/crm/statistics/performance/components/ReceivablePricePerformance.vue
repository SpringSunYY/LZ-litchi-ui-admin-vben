...
function renderLineChart() {
  ...
  const currentMonthReceivables = chartData.value.map((i) => i.currentMonthCount);
...
async function loadData() {
  ...
  try {
    ...
    const res = await getReceivablePricePerformance(params as any);
    ...
    totalStats.currentMonthReceivable = res.reduce((sum, item) => sum + item.currentMonthCount, 0);
    totalStats.lastMonthReceivable = res.reduce((sum, item) => sum + item.lastMonthCount, 0);
    totalStats.lastYearReceivable = res.reduce((sum, item) => sum + item.lastYearCount, 0);
    totalStats.totalReceivableAmount = res.reduce((sum, item) => sum + item.currentMonthCount, 0);
