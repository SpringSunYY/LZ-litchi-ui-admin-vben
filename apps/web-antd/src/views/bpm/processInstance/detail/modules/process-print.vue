...
async function parseFormFields() {
  ...
  for (const item of formFieldsObj) {
    ...
    printDataMap.value[fieldKey] = html as string;
    res.push({ id, name, html: html as string });
  }
...
function flattenAreaTree(
  list: Array<InfraAreaApi.Area & { children?: InfraAreaApi.Area[] }> = [],
...
async function loadPrintLookupMaps(formFieldsObj: FormFieldRule[]) {
  ...
  return {
    areaMap: flattenAreaTree(areaList as Array<InfraAreaApi.Area>),
    deptMap: new Map(
...
function initPrintDataMap() {
  ...
  printDataMap.value.startTime =
    (formatDate(printData.value.processInstance.startTime) as string) || '';
  printDataMap.value.endTime =
    (formatDate(printData.value.processInstance.endTime) as string) || '';
  ...
  printDataMap.value.printTime = printTime.value as string;
}
