...
const steps = [
  { title: $t('ai.workflow.message.stepBasicInfo'), validator: validateBasic },
  {
    title: $t('ai.workflow.message.stepWorkflowDesign'),
...
async function validateAllSteps() {
  ...
  try {
    ...
    currentStep.value = 0;
    throw new Error($t('ai.workflow.message.pleaseCompleteBasicInfo'));
  }
  ...
  try {
    ...
    currentStep.value = 1;
    throw new Error($t('ai.workflow.message.pleaseCompleteWorkflowInfo'));
  }
  return true;
}
...
async function handleSave() {
  try {
    ...
    message.warning(
      error.message || $t('ai.workflow.message.pleaseCompleteAllSteps'),
    );
  }
}
...
async function handleDeploy() {
  try {
    ...
    if (!formData.value.id) {
      await confirm($t('ai.workflow.message.confirmDeploy'));
    }
    // 校验所有步骤
    await validateAllSteps();
    ...
    message.success($t('ai.workflow.message.deploySuccess'));
    // 返回列表页
    await router.push({ name: 'AiWorkflow' });
  } catch (error: any) {
    console.error('deploy error:', error);
    message.warning(error.message || $t('ai.workflow.message.deployFailed'));
  }
}
...
async function handleStepClick(index: number) {
  try {
    ...
    message.warning($t('ai.workflow.message.pleaseCompleteCurrentStep'));
  }
}
      ...
      >
        ...
        <div class="flex w-48 items-center overflow-hidden">
          ...
          <span
            ...
            :title="
              formData.name ||
              $t('ui.actionTitle.create', [$t('ai.workflow.workflow')])
            "
          >
            {{
              formData.name ||
        ...
        <div class="flex w-48 items-center justify-end gap-2">
          <Button
            ...
            @click="handleDeploy"
          >
            {{ $t('ai.workflow.action.deploy') }}
          </Button>
          <Button type="primary" @click="handleSave">
            <span v-if="actionType === 'definition'">{{
              $t('ai.workflow.message.recover')
            }}</span>
