...
function handleOk() {
  if (!order.value) {
    message.warning($t('erp.saleOut.message.selectSaleOrder'));
    return;
  }
...
<template>
  <div>
    ...
    <Modal
      class="!w-[50vw]"
      v-model:open="open"
      :title="$t('erp.saleOut.message.selectSaleOrderTitle')"
      @ok="handleOk"
    >
      <Grid
        class="max-h-[600px]"
        :table-title="$t('erp.saleOut.message.saleOrderListTitle')"
      />
    </Modal>
  </div>
</template>
