...
function handleOk() {
  if (!order.value) {
    message.warning($t('erp.saleReturn.message.selectSaleOrder'));
    return;
  }
...
<template>
  <div>
    ...
    <Modal
      class="!w-[50vw]"
      v-model:open="open"
      :title="$t('erp.saleReturn.message.selectSaleOrderTitle')"
      @ok="handleOk"
    >
      <Grid
        class="max-h-[600px]"
        :table-title="$t('erp.saleReturn.message.saleOrderListTitle')"
      />
    </Modal>
  </div>
</template>
