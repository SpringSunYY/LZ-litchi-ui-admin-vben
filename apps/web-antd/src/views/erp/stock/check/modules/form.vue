...
const getTitle = computed(() => {
  if (formType.value === 'create') {
    return $t('ui.actionTitle.create', [$t('erp.stockCheck.stockCheck')]);
  } else if (formType.value === 'edit') {
    return $t('ui.actionTitle.edit', [$t('erp.stockCheck.stockCheck')]);
  } else {
    return $t('ui.actionTitle.detail', [$t('erp.stockCheck.stockCheck')]);
  }
});
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    try {
      ...
      message.error(
        error.message || $t('erp.stockCheck.message.childFormValidateFailed'),
      );
      return;
    }
