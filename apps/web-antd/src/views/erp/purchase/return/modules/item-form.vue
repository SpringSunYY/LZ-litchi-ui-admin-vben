...
function validate() {
  for (let i = 0; i < tableData.value.length; i++) {
    ...
    if (item) {
      if (!item.warehouseId) {
        throw new Error(
          $t('erp.purchaseReturn.message.warehouseRequired', [i + 1]),
        );
      }
      if (!item.count || item.count <= 0) {
        throw new Error(
          $t('erp.purchaseReturn.message.countRequired', [i + 1]),
        );
      }
    }
  }
  ...
  <Grid class="w-full">
    <template #warehouseId="{ row }">
      <Select
        ...
        :placeholder="$t('erp.purchaseReturn.message.pleaseSelectWarehouse')"
        :disabled="disabled"
        show-search
        class="w-full"
    ...
    <template #productId="{ row }">
      <Select
        ...
        :field-names="{ label: 'name', value: 'id' }"
        class="w-full"
        :placeholder="$t('erp.purchaseReturn.message.pleaseSelectProduct')"
        show-search
      />
    </template>
    <template #count="{ row }">
        ...
        :actions="[
          {
            ...
            danger: true,
            popConfirm: {
              title: $t('erp.purchaseReturn.message.confirmDeleteProduct'),
      ...
      <div class="border-border bg-muted mt-2 rounded border p-2">
        <div class="text-muted-foreground flex justify-between text-sm">
          <span class="text-foreground font-medium">{{
            $t('erp.purchaseReturn.message.totalLabel')
          }}</span>
          <div class="flex space-x-4">
            <span>
              {{ $t('erp.purchaseReturn.message.countLabel')
              }}{{ erpCountInputFormatter(summaries.count) }}
            </span>
            <span>
              {{ $t('erp.purchaseReturn.message.amountLabel')
              }}{{ erpPriceInputFormatter(summaries.totalProductPrice) }}
            </span>
            <span>
              {{ $t('erp.purchaseReturn.message.taxAmountLabel')
              }}{{ erpPriceInputFormatter(summaries.taxPrice) }}
            </span>
            <span>
              {{ $t('erp.purchaseReturn.message.taxAmountTotalLabel')
