...
function handleOk() {
  if (!order.value) {
    message.warning($t('erp.purchaseIn.message.selectPurchaseOrder'));
    return;
  }
...
<template>
  <div>
    ...
    <Modal
      class="!w-[50vw]"
      v-model:open="open"
      :title="$t('erp.purchaseIn.message.selectPurchaseOrderTitle')"
      @ok="handleOk"
    >
      <Grid
        class="max-h-[600px]"
        :table-title="$t('erp.purchaseIn.message.purchaseOrderListTitle')"
      />
    </Modal>
  </div>
</template>
